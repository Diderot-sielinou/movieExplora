...
export default function DetailPage() {
  ...
  useEffect(() => {
    ...
    const getsimilar = async (movieId) => {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/tv/${movieId}/similar?api_key=${import.meta.env.VITE_API_KEY}`);
        if (!response.ok) {
          throw new Error("Erreur lors de la récupération des détails du film");
        }
        const movieDetails = await response.json();
  ...
  const handleAddFavorie = (movie) => {
    ...
    if (tabFavorie) {
      ...
      console.log("local", JSON.parse(localStorage.getItem("localtabFavorie")));
    }
  };

  if (loading) {
    return (
      <div className="w-full text-4xl text-white text-center pt-20">
        chargent en cour ......
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="w-full text-4xl text-red-600 text-center pt-20">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="">
      ...
      {similar?.length>0?<MovieSilider movies={similar}>More Like this</MovieSilider>: <h2 className="text-center text-white text-4xl">no similar movie</h2> }
